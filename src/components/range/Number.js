...
export default class RangeComponent extends Input {
  ...
  render(content) {
    console.log('%cRenderer content', 'color:red', content);
    console.log('%cRenderer this', 'color:red', this);
    const originalRender = super.render();
    console.log('original render', originalRender);
    var newRender = originalRender.replace('type="text"', 'type="range"');
    newRender = newRender.replace('</input>', '</input><output class="rangeOutput"></output>');
    console.log('New render',newRender);
    return newRender;
  }

  attach(element) {
    console.log('Attaching element', element);
    console.log('Number of input elm', element.querySelectorAll('input'));
    console.log('Number of input elm', element.querySelectorAll('input[type=range]').length);
    const inputRangeElements = document.querySelectorAll('input[type=range]');

    // Loop through each input range element
    inputRangeElements.forEach(inputRange => {
        // Create an output element
        const outputElement = element.querySelector('.rangeOutput');

        // Set the initial value of the output to the value of the range
        outputElement.value = inputRange.value;

        // Update the output value when the range slider is changed
  ...
  static get serverConditionSettings() {
    return RangeComponent.conditionOperatorsSettings;
  }
